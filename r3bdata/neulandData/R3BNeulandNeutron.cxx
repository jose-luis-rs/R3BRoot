...
R3BNeulandNeutron::R3BNeulandNeutron(Int_t paddle, Double_t time, const TVector3& pos, const TVector3& pix)
    : fPaddle(paddle), fT(time), fPosition(pos), fPixel(pix)
{
}

R3BNeulandNeutron::R3BNeulandNeutron(const R3BNeulandHit& hit)
    : fPaddle(hit.GetPaddle()), fT(hit.GetT()), fPosition(hit.GetPosition()), fPixel(hit.GetPixel())
{
}

R3BNeulandNeutron::R3BNeulandNeutron(const R3BNeulandCluster& cluster) : R3BNeulandNeutron(cluster.GetFirstHit()) {}
