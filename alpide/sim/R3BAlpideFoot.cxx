...
R3BAlpideFoot::R3BAlpideFoot()
    : R3BAlpideFoot("")
{
}

R3BAlpideFoot::R3BAlpideFoot(const TString& geoFile, const TGeoTranslation& trans, const TGeoRotation& rot)
    : R3BAlpideFoot(geoFile, { trans, rot })
{
}

R3BAlpideFoot::R3BAlpideFoot(const TString& geoFile, const TGeoCombiTrans& combi)
    : R3BDetector("R3BAlpideFoot", kTRA, geoFile, combi)
    , fAlpidePoint(new TClonesArray("R3BAlpidePoint"))
    , fFootPoint(new TClonesArray("R3BTraPoint"))
    , fMap_Par(nullptr)
    , fAlpideGeo(nullptr)
    , fGeoversion(2024)
{
...
void R3BAlpideFoot::SetParameter()
{
    // Parameter Container
...
void R3BAlpideFoot::SetSpecialPhysicsCuts()
{
...
Bool_t R3BAlpideFoot::ProcessHits(FairVolume*)
{
...
TClonesArray* R3BAlpideFoot::GetCollection(Int_t iColl) const
{
    if (iColl == 0)
    {
        return fAlpidePoint;
...
// -----   Public method Print   ----------------------------------------------
void R3BAlpideFoot::Print(Option_t*) const
{
    R3BLOG(info, fAlpidePoint->GetEntriesFast() << " Alpide points registered in this event");
    R3BLOG(info, fFootPoint->GetEntriesFast() << " Foot points registered in this event");
}

// -----   Public method Reset   ----------------------------------------------
void R3BAlpideFoot::Reset()
{
    fAlpidePoint->Clear();
    fFootPoint->Clear();
    ResetParameters();
...
Bool_t R3BAlpideFoot::CheckIfSensitive(std::string name)
{
